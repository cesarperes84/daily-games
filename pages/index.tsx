        ...
        <div className={styles.grid}>
          {games.map(({ url, title, description, image, tags, stars }) => (
              <a href={url} target="_blank" className={styles.card}>
                  ...
                  <div>{tags.length ? tags.map((item) => <span className={styles.tag}>{item}</span>) : null}</div>
              </a>
          ))}
          </div>
          ...
          <div className={styles.container}>
            <div className={styles.about}>
              <h3>About Daily Games</h3>
                <p> You like to play games that offer new content or puzzle each day? Then you came to the right place and definitely daily games are for you in one place! Whether you enjoy daily crosswords, daily sudoku, or daily jigsaws. 
